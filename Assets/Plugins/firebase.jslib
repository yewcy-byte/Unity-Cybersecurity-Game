...
mergeInto(LibraryManager.library, {
    ...
    UpdateDocument: function (collectionPath, documentId, value, objectName, callback, fallback) {
        ...
        try {
            var data = JSON.parse(parsedValue);

            // If save data is moved to the object field, remove the old string field.
            if (Object.prototype.hasOwnProperty.call(data, "unityGameSaveObject") &&
                !Object.prototype.hasOwnProperty.call(data, "unityGameSaveJson")) {
                data.unityGameSaveJson = firebase.firestore.FieldValue.delete();
            }

            firebase.firestore().collection(parsedPath).doc(parsedId).update(data).then(function() {
